...
fn calculate_t(frame: usize, offset: f32, speed: f32, easing: &crate::core::Easing) -> f32 {
    ...
    match easing {
        ...
        crate::core::Easing::Custom(val) => ((phase.sin() + 1.0) / 2.0).powf(val.max(0.01)),
    }
}
    ...
    let items: Vec<ListItem> = app
        ...
        .map(|(index, item)| {
            ...
            if item.depth > 0 {
                ...
                indent.push_str(if item.is_last {
                    "└── "
                } else {
                    "├── "
                });
            ...
            let final_style = if item.is_dir {
...
pub fn draw_editor(f: &mut Frame, app: &mut App) {
    ...
    loop {
        ...
        for row in &current_page[ed.scroll_offset..] {
            let needed = if get_row_title(&row.fields[0].mapping).is_some() {
                3
            } else {
                2
            };
            if used_h + needed > available_height && visible_count > 0 {
                break;
            }
            used_h += needed;
            visible_count += 1;
    ...
    for row in &current_page[ed.scroll_offset..] {
        ...
        if used_h + needed > available_height && end_idx > ed.scroll_offset {
            break;
        }
        used_h += needed;
        end_idx += 1;

        layout_constraints.push(if is_header {
            Constraint::Length(1)
        } else {
            Constraint::Length(0)
        });
    ...
    for (r_idx, row) in visible_rows.iter().enumerate() {
        ...
        for (c_idx, field) in row.fields.iter().enumerate() {
            ...
            f.render_widget(
                Paragraph::new(if is_focused { "▌" } else { " " })
                    .style(Style::default().fg(if is_focused { BLUE } else { SURFACE1 })),
                field_layout[0],
            );
    ...
    let (preview_text, preview_icon) = match focused_mapping {
        ...
        FieldMapping::BranchColors | FieldMapping::BranchSpeed | FieldMapping::BranchEasing => {
            ...
            {
                if !is_valid_hex(hex) {
                    errors.push("Invalid Hex (use #RRGGBB)");
                }
            }
            ...
            (
                "├── └── │   (Branch formatting preview)".to_string(),
                String::new(),
            )
        }
        FieldMapping::DirColors
        | FieldMapping::DirIcon
        | FieldMapping::DirSpeed
        | FieldMapping::DirEasing => {
            for hex in current_theme
                .dir_colors
                .split(',')
                .map(|s| s.trim())
        ...
        FieldMapping::FileColors
        | FieldMapping::FileIcon
        | FieldMapping::FileSpeed
        | FieldMapping::FileEasing => {
            for hex in current_theme
                .file_colors
                .split(',')
                .map(|s| s.trim())
        ...
        | FieldMapping::RuleEasing(i) => {
            ...
            if glob::Pattern::new(&rule.glob).is_err() {
                errors.push("Invalid Glob Pattern");
            }
            for hex in rule
                .colors
                .split(',')
            ...
            let name = if rule.glob.contains('*') {
                rule.glob.replace('*', "example")
            } else {
                rule.glob.clone()
            };
    ...
    if !errors.is_empty() {
        ...
        if swatch_width > 1 {
            let mut swatch_spans: Vec<Span> =
                vec![Span::styled("  ◈ ", Style::default().fg(OVERLAY1))];
            ...
            f.render_widget(Paragraph::new(Line::from(swatch_spans)), preview_layout[3]);
        }
